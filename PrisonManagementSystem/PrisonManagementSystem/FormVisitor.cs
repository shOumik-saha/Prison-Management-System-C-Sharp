...
namespace PrisonManagementSystem
{
    public partial class FormVisitor : Form
    {
        ...
        private void btnReportRecords_Click(object sender, EventArgs e)
        {
            try
            {
                var nid = this.dgvVisitors.CurrentRow.Cells["VisitorNID"].Value.ToString();
                var name = this.dgvVisitors.CurrentRow.Cells["VisitorName"].Value.ToString();

                string sql = "delete from Visitor_Records where VisitorNID= '" + nid + "';";
                int count = this.Da.ExecuteDML(sql);

                if (count == 1)
                {
                    MessageBox.Show("Visitor Records " + name + " has been deleted.");
                }
                else
                {
